...
class SubscribeListViewController: UIViewController {
    ...
    func setListTableView() {
        ...
        Observable<[String]>.of(["오리부리", "지그재그", "abcd", "감자튀김", "닉주디", "I_need"])
            .bind(to: listTableView.rx.items(cellIdentifier: CellIdentifier.subscribeList, cellType: SubscribeListCell.self)) { (index:Int, element:String, cell:SubscribeListCell) in
